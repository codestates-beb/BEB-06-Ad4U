...
const ContractPrint = ({contractInfo, previewCheck, setTokenURI}) => {
    ...
    useEffect(() => {
        ...
        .then(async (canvas) => {
            ...
            pdf.save("contract_"+Date.now()+".pdf"); // 사용자 계약서 저장
            const datauri = pdf.output("datauristring"); // encrypt를 위한 datauri로 
            console.log(datauri)

            // 암호화
            const secretKey = process.env.REACT_APP_SECRET_KEY;
            console.log(secretKey)
            const encrypted = crypto.AES.encrypt(datauri, secretKey).toString(); //속도 제일 빠름
