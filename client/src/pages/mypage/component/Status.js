...
const Status = ({ adList, setStatus }) => {
  const [count, setCount] = useState({ status0: 0, status1: 0, status2: 0, status3: 0, status4: 0, status5: 0 })

  useEffect(() => {
    let status0 = 0, status1 = 0, status2 = 0, status3 = 0, status4 = 0, status5= 0;
    adList.forEach(el => {
      ...
      if (el.status === 5) return status5++;
      else return ;
    })
    setCount({ status0, status1, status2, status3, status4, status5 });
  }, [adList]);
    ...
    <Container className='status_container'> 
      <div className="status-content_card-container">     
        ...
        <Row className='status_count'>
          ...
          <Col>{count.status2 + count.status3}</Col>
          <Col>{count.status4}</Col>
          <Col>{count.status5}</Col>
        </Row>
      </div>
    </Container>
  );
